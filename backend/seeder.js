...
(async () => {
    ...
    console.log('🚀 Seeding 5 Perfect Life-Cycle Records...');
    ...
    try {
        ...
        await pool.query('INSERT INTO application (ApplicationID, FirstName, LastName, PreferredRole, ApplicationDate) VALUES (1, "Arunjit", "Singh", "Software Developer", "2026-03-20")');
        ...
        await pool.query('INSERT INTO resumeskills (ApplicationID, Skill) VALUES (1, "React"), (1, "Node.js")');
        await pool.query('INSERT INTO resumeprojects (ApplicationID, Project) VALUES (1, "E-commerce App")');

        // 👥 Life-Cycle 2: Screened Candidate (Bob)
        await pool.query('INSERT INTO application (ApplicationID, FirstName, LastName, PreferredRole, ApplicationDate) VALUES (2, "Bob", "Smith", "Marketing Lead", "2026-03-15")');
        ...
        await pool.query('INSERT INTO application (ApplicationID, FirstName, LastName, PreferredRole, ApplicationDate) VALUES (3, "Charlie", "Davis", "Sales Manager", "2026-03-10")');
        ...
        await pool.query('INSERT INTO application (ApplicationID, FirstName, LastName, PreferredRole, ApplicationDate) VALUES (4, "Deepa", "Verma", "HR Specialist", "2026-03-01")');
        ...
        await pool.query('INSERT INTO offer (OfferID, CandidateID, SalaryID, ContractID, OfferStatus) VALUES (4, 4, 4, 4, "Accepted")');
        await pool.query('INSERT INTO awarded (OfferID, CandidateID, AwardedDate) VALUES (4, 4, "2026-03-10")');
        await pool.query('INSERT INTO training (CandidateID, TrainingStatus, TrainingStartDate, Insights) VALUES (4, "Ongoing", "2026-03-15", "Active participation in orientation")');

        // 🏢 Life-Cycle 5: Full Employee (Eva)
        await pool.query('INSERT INTO application (ApplicationID, FirstName, LastName, PreferredRole, ApplicationDate) VALUES (5, "Eva", "Jain", "Financial Analyst", "2026-02-01")');
        ...
        // Raise a complaint for the employee
        await pool.query("INSERT INTO complaint (ComplaintID, EmployeeID, ComplaintDateTime, Description, ComplaintStatus, Priority) VALUES (2, 5, '2026-03-26 14:15:00', 'Access to the finance portal is restricted', 'Open', 'High')");
